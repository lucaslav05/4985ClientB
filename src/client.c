...
static int setup_socket(int *sockfd, struct sockaddr_in *serveraddr)
{
...
static void send_acc_login(const int *sockfd, struct account *client)
{
    struct Message   msg;
    ...
    lgn.username = (uint32_t)*client->username;
    lgn.password = (uint32_t)*client->password;

    msg.packet_type      = ACC_LOGIN;
    msg.protocol_version = 1;
    msg.sender_id        = (uint16_t)client->uid;
...
// static void send_acc_create(void) {}
...
int main(void)
{
    ...
    client.uid      = 1;    // temp just so it builds
